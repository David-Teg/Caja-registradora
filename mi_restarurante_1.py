...
def click_boton(numero):
    ...
    """
    Agrega el n mero recibido como par metro al valor actual de la variable
    operador y lo muestra en el visor de la calculadora. No devuelve ning n
    valor.
    """
...
def borrar():
    """
    Limpia el visor de la calculadora y borra el valor actual de la variable
    operador. No devuelve ning n valor.
    """
    global operador
    operador = ''
    visor_calculadora.delete(0, END)   
...
def obtener_resultado():
    """
    Obtiene el resultado de la operaci n actual en el visor de la calculadora.
    Limpia el visor y coloca el resultado en  l.
    Luego, borra la variable operador.
    """
    global operador
...
def revisar_check():
    """
    Revisa los checks de cada tipo de alimento y bebienda, si esta activado, 
    habilita el cuadro de texto correspondiente y si no, lo deshabilita y 
    pone el valor en 0.
    """
...
def total():
        ...
        Primero calcula el sub_total de cada tipo de alimento, luego
        calcula el sub_total total, el impuesto y el total.
        Luego actualiza los valores de los labels de costo de cada tipo
        de alimento, el sub_total, el impuesto y el total.
        """
        sub_total_comida = 0
        p = 0
...
def recibo():
    """
    Muestra el recibo con los items seleccionados y sus correspondientes
...
def guardar():
    """
    Guarda el recibo actual en un archivo de texto.
    
    Abre un di logo para elegir el nombre y ubicaci n del archivo y
    escribe el contenido del visor de recibo en el archivo.
    
    Muestra un mensaje de confirmaci n al usuario una vez que el archivo
    ha sido guardado.
    """
...
def resetear():
    """
    Esta funcion resetea todos los valores de los campos de texto, 
    variables de control, costo de cada tipo de alimento y el costo total
    de la orden. Tambien deshabilita todos los cuadros de texto.
    """
    texto_recibo.delete(1.0, END)
...
def guardar_comida_precios_csv():
    # Nombre del archivo
    ...
    if not nombre_archivo:  # Si el usuario cancela, no hacer nada
        return
    
    try:
        # Abrimos el archivo en modo de agregado
        with open(nombre_archivo, mode='a', newline='', encoding='utf-8') as archivo_csv:
            escritor_csv = csv.writer(archivo_csv)
            
            # Escribimos encabezados si el archivo está vacío
            archivo_vacio = archivo_csv.tell() == 0
            if archivo_vacio:
                escritor_csv.writerow(["Comida", "Precio"])
            
            # Escribir nombres de comida y precios
            for idx, comida in enumerate(texto_comida):
                cantidad = comida.get()
                if cantidad != "0":  # Solo guardar si la cantidad no es 0
                    escritor_csv.writerow([lista_comida[idx], float(cantidad) * precios_comida[idx]])
        
        messagebox.showinfo("Información", "Nombres de comida y precios guardados exitosamente en el archivo CSV.")
...
boton_guardar_comida = Button(panel_botones,
                            text="Guardar Comida",
                            font=('Dosis', 14, 'bold'),
                            fg='white',
                            bg='azure4',
                            bd=1,
                            width=12,
                            command=guardar_comida_precios_csv)
boton_guardar_comida.grid(row=1, column=1)
