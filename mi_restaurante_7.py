...
def inicializar_archivo():
    try:
        with open(archivo_usuarios, mode='a', newline='', encoding='utf-8') as archivo:
            ...
            archivo_vacio = archivo.tell() == 0
            if archivo_vacio:
                escritor_csv.writerow(["ID", "Nombre", "Apellido", "Documento", "Fecha de Nacimiento", "Teléfono", "Domicilio"])
    except Exception as e:
        messagebox.showerror("Error", f"No se pudo inicializar el archivo.\n{e}")
...
def registrar_usuario():
    ...
    if not (nombre and apellido and documento and fecha_nacimiento and telefono and domicilio):
    ...
    try:
        ...
        with open(archivo_usuarios, mode='r', encoding='utf-8') as archivo:
            ...
            id_usuario = len(lineas)  # El ID será el número de líneas
        
        # Guardar el nuevo usuario
        with open(archivo_usuarios, mode='a', newline='', encoding='utf-8') as archivo:
            escritor_csv = csv.writer(archivo)
            escritor_csv.writerow([id_usuario, nombre, apellido, documento, fecha_nacimiento, telefono, domicilio])
        
        messagebox.showinfo("Éxito", f"Usuario {nombre} registrado correctamente.")
        
        # Limpiar campos
        for entry in [entry_nombre, entry_apellido, entry_documento, entry_fecha_nacimiento, entry_telefono, entry_domicilio]:
            entry.delete(0, END)
    except Exception as e:
...
def listar_usuarios():
    try:
        ...
        ventana_listado.title("Listado de Usuarios")
        
        texto = Text(ventana_listado, wrap=WORD, width=100, height=30)
        texto.pack(pady=10)
...
Label(root, text="Documento").grid(row=2, column=0)
entry_documento = Entry(root)
entry_documento.grid(row=2, column=1)

Label(root, text="Fecha de Nacimiento").grid(row=3, column=0)
entry_fecha_nacimiento = Entry(root)
entry_fecha_nacimiento.grid(row=3, column=1)

Label(root, text="Teléfono").grid(row=4, column=0)
entry_telefono = Entry(root)
entry_telefono.grid(row=4, column=1)

Label(root, text="Domicilio").grid(row=5, column=0)
entry_domicilio = Entry(root)
entry_domicilio.grid(row=5, column=1)

# Botones
Button(root, text="Registrar Usuario", command=registrar_usuario).grid(row=6, column=0, pady=10)
Button(root, text="Listar Usuarios", command=listar_usuarios).grid(row=6, column=1)
